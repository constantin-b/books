...
abstract class Page_Abstract implements Page_Interface {
	...
	public function get_db() {
		return $this->admin->get_plugin()->get_db();
	}
	 ...
	 * @param string $name
	 */
	public function set_wp_nonce( $action, $name ) {
	 ...
	 * @return string|void
	 */
    public function get_url( $params = [], $with_nonce = true, $echo = true ) {
        $url = add_query_arg( $params, $this->get_menu_page( false ) );
	    if( $with_nonce ){
	        if( !$this->nonce_action || !$this->nonce_name ){
	            trigger_error( 'Nonce action and name not set. Could not generate page URL with nonce.', E_USER_WARNING );
                return false;
	        }

	        $url = wp_nonce_url( $url, $this->nonce_action, $this->nonce_name );
        }
